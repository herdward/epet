...
let pet_tests =
  [
    ...
    ( {| update food amount for sausage two times, food amount should be 1|} >:: fun _ -> assert_equal 1 
    (food_amount (update_food_amount(update_food_amount (get_good_food ocat "sausage")))));

    ( {| update food amount for sausage three times, food amount should be 0 |} >:: fun _ -> assert_equal 0 (food_amount(update_food_amount(update_food_amount(update_food_amount(get_good_food ocat "sausage"))))));

    ( {| update food amount for cod three times, food amount should be 0 |} >:: fun _ -> assert_equal 0 (food_amount(update_food_amount(update_food_amount(update_food_amount(get_good_food ocat "cod"))))));

    
    ( {| update food amount for milk three times, food amount should be 0 |} >:: fun _ -> assert_equal 0 (food_amount(update_food_amount(update_food_amount(update_food_amount(get_good_food ocat "milk"))))));



        
    ( {| update food amount for chocolate three times, food amount should be 0 |} >:: fun _ -> assert_equal 0 (food_amount(update_food_amount(update_food_amount(update_food_amount(get_bad_food ocat "chocolate"))))));

    ( {| update food amount for grapes three times, food amount should be 0 |} >:: fun _ -> assert_equal 0 (food_amount(update_food_amount(update_food_amount(update_food_amount(get_bad_food ocat "grapes"))))));
    ( {| update food amount for egg three times, food amount should be 0 |} >:: fun _ -> assert_equal 0 (food_amount(update_food_amount(update_food_amount(update_food_amount(get_bad_food ocat "egg"))))));
