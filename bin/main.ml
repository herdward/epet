...
let cleanencounter (pet : pet) : pet =
  ...
  | "3" -> begin
      try
        let updatedpet = Pet.update_pet_hygiene pet 100 in
...
let rec select_pet (state : State.state) (player : Player_state.player_state) :
  ...
  | petname -> (
      try
        ...
        (* bro how do we get rid of this error, we basically have to explicitly
           list all of them since initially all of them are "None"*)
        let new_state =
          {
            state with
...
(* Game loop *)
(* Lwt promises are used so that while in the pet_game_loop, control can
   actually be passed back to the player_game_loop*)
let rec pet_game_loop (state : State.state)
    (player_state : Player_state.player_state) :
    (State.state * Player_state.player_state) Lwt.t =
  if state = init_pet_state then (
    ...
    | Some health ->
        if health <= 0 then (
          ANSITerminal.print_string [ ANSITerminal.red ]
            ("\n\nOh no! " ^ State.get_pet_name state
           ^ " got OOFED (UNALIVED). You lost :(\n");
          Unix._exit 0)
        else Lwt.return (new_pet_state, new_player_state)
...
let rec player_game_loop (player_state : Player_state.player_state) : unit Lwt.t
  ...
  if player_state = init_player_state then begin
    ...
    match Stdlib.read_line () with
    | input ->
        let new_state = { player_state with name = input } in
    ...
    | None ->
        ...
        in
        (* callback function that updates the pet_state field of the
           player_state*)
    ...
    | Some old_pet_state ->
        (* callback function that updates the pet_state field of the
           player_state using the output of pet_game_loop, and prints info*)
        pet_game_loop old_pet_state player_state
        >>= fun (new_pet_state, new_player_state) ->
        Player_state.print_player_info new_player_state
        |> Lwt.return |> Lwt.ignore_result;
        let updated_player_state =
          { new_player_state with pet_state = Some new_pet_state }
        in
